...
class FCNMaskHead(nn.Module):
    ...
    def get_seg_masks(self, mask_pred, det_bboxes, det_labels, rcnn_test_cfg,
        ...
        # assert num_chunks == N, "come on!, implement a batch version! yf&ly!!!"
        ...
        for inds in chunks:
            # assert len(inds) == 1, 'implement a batch version! yf&ly!!!'
        ...
        for i in range(N):
            rle = mask_util.encode(
                np.array(im_mask[i][:, :, np.newaxis].cpu(), order='F'))[0]
            if det_obj_ids is not None:
                if det_obj_ids[i] >= 0:
                    obj_segms[det_obj_ids[i]] = rle
            else:

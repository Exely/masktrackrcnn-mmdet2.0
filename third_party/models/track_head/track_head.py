...
import numpy as np
from mmdet.models.losses import cross_entropy, accuracy
...
class TrackHead(nn.Module):
    ...
    def compute_comp_scores(self, match_ll, bbox_scores, bbox_ious, label_delta, add_bbox_dummy=False):
        ...
        label_delta = label_delta.to(device)
        if add_bbox_dummy:
            bbox_iou_dummy = torch.ones(bbox_ious.size(0), 1, device=device) * self.bbox_dummy_iou
            bbox_ious = torch.cat((bbox_iou_dummy, bbox_ious), dim=1)
            label_dummy = torch.ones(bbox_ious.size(0), 1, device=device)
    ...
    def loss(self,
             ...
             id_weights):
        losses = dict()
        if isinstance(match_score, torch.Tensor):
            device = match_score.device
        else:
            device = match_score[0].device
        if self.dynamic:
            n = len(match_score)
            x_n = [s.size(0) for s in match_score]
            ids = torch.split(ids, x_n, dim=0)
            loss_match = torch.tensor([0.], device=device)
            match_acc = torch.tensor([0.], device=device)
            n_total = 0
            batch_size = len(ids)
            for score, cur_ids, cur_weights in zip(match_score, ids, id_weights):
                ...
                n_total += n_valid
                loss_match += cross_entropy(
                    score, cur_ids, weight=cur_weights, reduction='mean')
            ...
            if match_score is not None:
                ...
                losses['loss_match'] = cross_entropy(
                    match_score, ids, weight=id_weights)
